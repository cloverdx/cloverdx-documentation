...
| Advanced |  |  |  |
| Preserve file timestamps | no | Set the last modified date of moved files and directories to the date of source files and directories.  With **Preserve, fail if unable**, the target timestamps will always be set according to the source.  With **Preserve if supported**, the target timestamps will be set according to the source, if the target file system supports setting last modified date.  With **No special handling**, the timestamp will depend on the file system and its move operation behavior.  **Note:** this depends on the ability of source file system to provide the file date, and the ability of target file system to set last modified date (see [file system support overview](common-attributes-fo.md#preserve-file-timestamps)). | Preserve if supported (default) \| Preserve, fail if unable \| No special handling |
