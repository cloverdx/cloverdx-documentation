...
> [!IMPORTANT]
> Remember that the object notation (e.g., `arg.isnull()`) cannot be used with the language keywords listed among these functions - `cast`, `evalExpression`, `iif`, `isnull`, `nvl`, `nvl2`, `printErr`, `printLog` and `raiseError`. The remaining **Miscellaneous** functions support it.
>
> For more information about object notation, see [Calling a function](language-reference-ctl2.md#calling-a-function).
...
Example 319. Usage of currentTimeMillis()`long millis = currentTimeMillis();`
#### evalExpression
...
Example 320. Example of using a custom function:
...
Example 324. Usage of getEnvironmentVariables()`string envPath = getEnvironmentVariables()["PATH"];`
...
Example 325. Usage of getJavaProperties()`string operatingSystem = getJavaProperties()["os.name"];`
...
Example 326. Usage of getParamValue`string datainDir = getParamValue("DATAIN_DIR"); // will contain "./data-in"`
...
Example 327. Usage of getParamValues`string datainDir = getParamValues()["DATAIN_DIR"]; // will contain "./data-in"`
...
Example 328. Usage of getRawParamValue`string datainDir = getRawParamValue("DATAIN_DIR"); // will contain "${PROJECT}/data-in"`
...
Example 329. Usage of getRawParamValues`string datainDir = getRawParamValues()["DATAIN_DIR"]; // will contain "${PROJECT}/data-in"`
...
Example 331. Usage of hashCode The function `hashCode(5)` returns some number.
#### iif
...
Example 337. Sample property file
...
Example 338. Usage of parseProperties
...
Example 340. Usage of raiseError`raiseError("The error message")`
...
Example 341. Usage of resolveParams The usage of the function `resolveParams()` is necessary if the string containing a parameter is created at runtime:
...
Example 342. Usage of sleep The function `sleep(5000)` will sleep for 5 seconds.
#### toAbsolutePath
...
Example 343. Usage of toAbsolutePath The function `toAbsolutePath("graph")` will return for example `C:/workspace/doc_project/graph`.
