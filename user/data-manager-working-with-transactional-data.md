...
When applying an action to multiple selected rows, the selected rows do not all need to have the same status. If some rows are already in the status that the action would set, those rows are left unchanged.

For example, when you approve selected rows:

- rows that are already approved stay approved
- rows that can still be approved are changed to *Approved*

This allows you to select a larger group of rows and apply the action without checking each row individually.

Rows that cannot be changed by the selected action are still not affected. For example, an action that moves rows forward in the workflow cannot be used to move rows back to an earlier status.
...
- **Category**: an optional category used to organize data sets in Data Manager and Server UI. You can select an existing category or create a new one. See [Data set categories](data-manager-introduction.md#data-set-categories) for more details.
